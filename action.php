...
if (isset($_POST['signup'])) {
	...
	if (mysqli_num_rows($run)>0) {
		echo "<script>alert('USERNAME ALREADY EXIST PLEASE ENTER VALID USERNAME')</script>";
	}else{
...
if (isset($_POST['search'])) {
  ...
  $cdate = date("Y-m-d");
    ...
    $current_timestamp = strtotime($cdate);
    if ($date_timestamp < $current_timestamp) {
        echo "<script>alert('Please select a date that is not in the past');</script>";}
  else{
	$sql="SELECT * FROM booking_det WHERE jdate='$date'AND bfrom='$from'AND bto='$to'AND vacant";
	$run=mysqli_query($con,$sql);
    if (mysqli_num_rows($run)>0) {
...
if (isset($_POST['booking'])) {
  ...
  if ($no_p<1 || $no_p>$vacant){
    echo "Enter valid number of seats";
  } else {
      ...
      if (mysqli_num_rows($run) > 0) {
          ...
          if ($run) {
              ...
              $sql = "SELECT MAX(tid) AS last_ticket_id FROM ticket WHERE bus_id='$bid' AND jdate='$jid'";
              $run = mysqli_query($con, $sql);
              $row = mysqli_fetch_assoc($run);
              $last_ticket_id = $row['last_ticket_id'];

              $sql = "SELECT seat_no FROM ticket WHERE tid='$last_ticket_id'";
              $run = mysqli_query($con, $sql);
              $row = mysqli_fetch_assoc($run);
              $last_seat_no = $row['seat_no'];

              $last_seat_numbers = explode(" ", $last_seat_no);
              $last_seat = end($last_seat_numbers);
              ...
              if (mysqli_num_rows($run) > 0) {
                  ...
                  $sql="INSERT INTO `ticket` (`tid`, `bus_id`, `uid`, `seat_no`, `no_seat`, `ticket_status`, `jdate`, `booking_date`, `pname`) VALUES (NULL, '$bid', '$uid', '$tseatno', '$no_p', 'Confirm', '$jid', '$date', '$name')";
                  $run=mysqli_query($con,$sql);

                  if ($run) {
                      echo "Your Ticket is Confirmed. Thank you";
                  }
              }
          }
      } else {
          ...
          if ($run) {
              ...
              // Retrieve the last booked ticket's seat number
              $sql = "SELECT MAX(tid) AS last_ticket_id FROM ticket WHERE bus_id='$bid' AND jdate='$jid'";
              $run = mysqli_query($con, $sql);
              $row = mysqli_fetch_assoc($run);
              $last_ticket_id = $row['last_ticket_id'];

              // Retrieve the last booked ticket details
              $sql = "SELECT seat_no FROM ticket WHERE tid='$last_ticket_id'";
              $run = mysqli_query($con, $sql);
              $row = mysqli_fetch_assoc($run);
              $last_seat_no = $row['seat_no'];
...
if (isset($_POST['tbkd'])) {
   $bid=$_POST['bid'];
   $pdate = date("Y-n-j", strtotime($_POST['date'])); 
   $sql2="SELECT * FROM bus_details WHERE bus_id='$bid'";
          $run2=mysqli_query($con,$sql2);
          $row1=mysqli_fetch_array($run2);
          if($row1){
